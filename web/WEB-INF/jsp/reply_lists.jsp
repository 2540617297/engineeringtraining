    ...
    <script>
        function replydel(replyid) {
                if (confirm("确认删除该条记录吗?"))
                    location.href = "../adminController/replydel?replyid="+replyid;
        }
    </script>
</head>
<body>
                                    ...
                                    <div class=" panel-collapse collapse <%if(lists.get(0).getNavfname().equals("用户管理专区")){%>in<%}%>"  id="panel<%=lists.get(0).getNavss().getNavsid()%>">
                                        <div class="panel-body" style=" border: none; padding:0px 15px 0px 5px;">
                                            <ul class="nav">
                                                ...
                                                %>
                                                <li><a <%if(lists.get(i).getNavss().getNavsname().equals("评论管理")){%>style="background-color: #1A2226;color: #ffffff;" <%}%> href="<%=lists.get(i).getNavss().getNavsherf()%>">
                ...
                <div class="col-lg-10" id="content-right" style="height: auto ;position: relative">
                    <div id="content-header">
                        <h3 style="margin:0px;" class="page-header">
                            reply管理
                            <small>信息</small>
                            <a href="equipment-information.jsp" class="pull-right" style="font-size: xx-small"><span class="glyphicon glyphicon-tag"></span>reply管理</a>
                        </h3>
                    </div>
                    ...
                    <div id="content" style="margin-bottom: 50px;height: 1056px;">
                        ...
                        <div style="background-color: #ffffff;">
                            <form action="../adminController/replylists" role="form" class="form-horizontal">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><%=reply.getLastid()%></td>
                                            <td><a href="../postController/postshow?postid=<%=reply.getPostid()%>&recordpage=../adminController/replylists"><%=reply.getPostInformation().getPosttitle()%></a></td>
                                            <td><%=reply.getReplytime()%></td>
                                            <td>
                                                <a href="#" class="btn btn-danger  btn-xs" onclick="replydel(<%=reply.getReplyid()%>)">删除</a>
                                            </td>
                                        </tr>
                                    </tbody>
                                    <%}%>
                            ...
                            <div class="text-center">
                                <ul class="pagination">
                                    <li><a class="btn btn-default" href="../adminController/replylists?getnowpage=0<%if(question!=null){%>&question=<%=question%><%}%>" >首页</a></li>
                                    <li><a class="btn btn-default" href="../adminController/replylists?getnowpage=<%=nowpage-1%><%if(question!=null){%>&question=<%=question%><%}%>" >上一页</a></li>
                                    <li><a class="btn btn-default" href="../adminController/replylists?getnowpage=<%=nowpage+1%><%if(question!=null){%>&question=<%=question%><%}%>">下一页</a></li>
                                    <li><a class="btn btn-default" href="../adminController/replylists?getnowpage=<%=allpagenum%><%if(question!=null){%>&question=<%=question%><%}%>">页尾</a></li>
                                </ul>
                            </div>
